...
public class RssServlet extends SageServlet {
    ...
    protected void doServletGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            if ( req.getPathInfo() != null ) {
               ...
               {
                   Channel channel=new Channel("Sage Recording Schedule",
                           baseURL+"/RecordingSchedule",
                           "The programs to be recorded by Sage in the next few days");
                   ...
                   Channel channel=new Channel("Sage Intelligent Suggestions",
                           baseURL+"/IRSuggestions",
                           "Programs Sage thinks that you might be interested in recording over the next few days");
                   rss.addChannel(channel);
                   Object filelist=SageApi.Api("GetSuggestedIntelligentRecordings");
                   AddShowsToChannel(baseURL,channel,filelist,req,resp);
                   
               } else if ( req.getPathInfo().equals("/Conflicts") ) {
                   Channel channel=new Channel("Sage Recording Conflicts",
                           baseURL+"/Conflicts",
                           "Programs that will not be recorded because they conflict with other recordings");
                   rss.addChannel(channel);
                   ...
                   Channel channel=new Channel(searchName,
                           baseURL+"/Search?"+req.getQueryString(),
                           "Search results from Sage Database");
                   rss.addChannel(channel);
                   AddShowsToChannel(baseURL,channel,searchResults,req,resp);
               } else if ( req.getPathInfo().startsWith("/SystemMessages") ) {
                   Channel channel=new Channel("Sage System Messages",
                           baseURL+"/SystemMessages",
                           "System messages reported by SageTV");
                   rss.addChannel(channel);
    ...
    private void AddShowsToChannel(String baseURL,Channel channel,Object airings,HttpServletRequest req, HttpServletResponse resp)
    throws Exception 
    {
        channel.setPubDate(new Date());
        channel.setTtl(SageApi.GetIntProperty("nielm/webserver/rss_ttl_mins",360));// every 6 hrs
        ...
        for ( int i = 0 ; i < SageApi.Size(airings); i ++ ){
            ...
            String dateStr=DateFormat.getDateInstance().format(startDate)+" "+
            ...
            if ( categ!=null && categ.length()>0){
                ...
                if ( subcateg!=null && subcateg.length()>0)
                    description=description+"/"+subcateg;
            ...
            if ( airing.idType == Airing.ID_TYPE_MEDIAFILE){
                ...
                if ( file!=null && file.canRead() && file.length() > 0) {
                    ...
                    if (mimeType == null)
                    {
                        // avoid NPE for missing mime type
                        mimeType = "text/plain";
                    }
    ...
    private void AddConflictsToChannel(String baseURL,Channel channel,HttpServletRequest req, HttpServletResponse resp)
    throws Exception 
    {
        channel.setPubDate(new Date());
        channel.setTtl(SageApi.GetIntProperty("nielm/webserver/rss_ttl_mins",360));// every 6 hrs
    ...
    private void AddSystemMessagesToChannel(String baseURL,Channel channel,HttpServletRequest req, HttpServletResponse resp)
    throws Exception 
    {
        channel.setPubDate(new Date());
        channel.setTtl(SageApi.GetIntProperty("nielm/webserver/rss_ttl_mins",360));// every 6 hrs
        ...
        for (int i = 0; i < SageApi.Size(messageList); i++ )
        {
            ...
            // Key is what makes the message unique
            // Uid tells the RSS reader what's unique, so it must change when a new occurrence happens
            ...
            // Safari uses the Guid as the link, so not only must it be unique, it must be a valid url with anchor
            // messageUid and messageKey have some redundant information but it doesn't really matter
    ...
    private void ReturnFeed(RSS rss,HttpServletRequest req, HttpServletResponse resp) 
    throws Exception {
