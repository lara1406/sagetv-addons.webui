 ...
 *
 */
public class SearchServlet extends SageServlet {
    
    /**
     * 
     */
    private static final long serialVersionUID = -3875835578723435724L;
    ...
    throws Exception {
        ...
        // determine time range, default to current and future airings
        ...
        try {
            ...
            if ((search.getSearchString()!= null) || (search.getFavoriteId() != null)) {           
                ...
                if ( search.getSearchString()!=null || search.getFavoriteId() != null) {
                    if (search.isSearchTypeMediaFiles()) {
                        ...
                        if (search.getFavoriteId() != null || search.isSearchTypeAirings()) {
                            if ("-1".equals(timeRangeStr)) { // all airings
                                
                            } else if ("-999".equals(timeRangeStr)) { // past airings
                                out.println(" in the Past");
                            } else if ("0".equals(timeRangeStr)) { // future airings
                                out.println(" in the Future");
                            } else {
                                // print duration
                                long duration=(search.getEndtime().longValue()-search.getStarttime().longValue())/1000/60/60;
                                if (duration > 48 && duration%24==0 )
                                    out.println(" within the next "+duration/24+" days ");
                                else
                                    out.println(" within the next "+duration+" hours ");
                            }
                        }  
                    }
                }
                ...
                if (search.isSearchTypeTVFiles() || search.getFavoriteId() != null || search.isSearchTypeAirings() ) {
                    ...
                    if ( ! (searchType.equals(Search.SEARCH_TYPE_AIRINGS)
                             && startTime==0)) { 
                        // past airings
                    ...
                    if ( numTV>0|| numPast>0 || numFuture>0)
                    {
                        ...
                        if (numTV>0 ) {
                            out.println(" <a href=\""
                                        +URL_nottyperange+"&amp;searchType="+Search.SEARCH_TYPE_TV_FILES
                                        +"\">"+numTV+" recorded TV files</a>");
                        }
                        if ( numPast >0 ){
                            if (numTV>0 )
                                out.print(";");
                            out.println(" <a href=\"" 
                                    +URL_nottyperange+"&amp;searchType="+Search.SEARCH_TYPE_AIRINGS+"&amp;TimeRange=-999"
                                    +"\">"+numPast+" past Airings</a>");
                        
                        }
                        if ( numFuture>0){
                            if (numTV >0 ||numPast >0 )
                                out.print(";");
                            out.println(" <a href=\"" 
                                    +URL_nottyperange+"&amp;searchType="+Search.SEARCH_TYPE_AIRINGS+"&amp;TimeRange=0"
                                    +"\">"+numFuture+" future Airings</a>");
                        }
                        out.println(")</p>");
                    }
                ...
                if ( numprogs > 0){
                    ...
                    boolean showFileSize=( GetOption(req,"ShowFileSize","true").equalsIgnoreCase("true")
                                            && ! search.isSearchTypeAirings());
                    
                    int res_per_page=0;
                    String s=req.getParameter("pagelen");
                    if ( s != null){
                        if ( s.equals("inf"))
                            res_per_page=Integer.MAX_VALUE;
                        else try {
                    ...
                    AiringList.PrintPagedGroupedAiringList(this,
                                                    ...
                                                    req,
                                                    groupMethod,
                                                    // only show checkbox for TV shows.
                                                    (search.isSearchTypeTVFiles()
                                                     || search.isSearchTypeAirings()),
                                                    true, 
                                                    showFileSize, 
                                                    RecSchedList, 
                ...
                // no parameter, just output search dialog
            ...
            // search box:
            out.println("<div id=\"options\" class=\"options\" style=\"display: block;\">"+
                    "<form name=\"SearchForm\" method=\"get\" action=\""+req.getRequestURI()+"\">");
            out.print("<table>");
            if (search.getFavoriteId() == null) {
                ...
                if ( search.getSearchString() != null) 
                    out.println("value=\""+Translate.encode(search.getSearchString())+"\"/>");  
                else 
                    out.println("value=\"\"/>");
            } else {
                // put faveID as hidden
            ...
            if (search.getFavoriteId() == null) {
                ...
                if (search.isVideo() || !search.isSearchTypeMediaFiles()) 
                    out.print(" checked=\"checked\" ");
                out.println("/>Video<br/>");       
                out.print("<input type=\"checkbox\" name=\"DVD\" ");
                if (search.isDVD() || !search.isSearchTypeMediaFiles()) 
                    out.print(" checked=\"checked\" ");
                out.println("/>DVD<br/>");
            ...
            if ( search.getFilename() != null) 
                out.println("value=\""+Translate.encode(search.getFilename())+"\"/>");  
            else 
                out.println("value=\"\"/>");
            out.print("</td>\r\n");
            ...
            out.println("<tr><td colspan=\"2\">Filter results:</td></tr>"+
                    "<tr><td>\r\n"+
            ...
            if ( Categories!= null )
                Categories_l=Arrays.asList(Categories);
            else
                Categories_l=new Vector<String>();

            out.print("   <option value=\"**Any**\" ");
            if ( Categories == null || Categories.length==0
                    || Categories[0].equals("**Any**"))
                out.print(" selected=\"selected\"");
            ...
            for ( int i=0;i<numcategs;i++) {
                ...
                if ( Categories != null && Categories.length>0 && Categories_l.contains(categ.toString()))
                    out.print(" selected=\"selected\"");
            ...
            "</td></tr>\r\n<tr><td>\r\n");
            try {
                SageApi.Api("IsAiringHDTV", new Object[]{new Object()}); // verify HDTV API exists
            ...
            opt=req.getParameter("pagelen"); // passed param overrides cookie
            if ( opt==null) {
    ...
    static final String[][] TIME_FILTER_OPTS=new String[][] {
        {"-1","anytime"},
        {"-999","in the past"},
        {"0","in the future"},
        {"24","within 24 hrs"},
    ...
    static final String[][] FIRST_RUNS_OPTS=new String[][] {
        {"any","Any"},
        {"set","First Runs"},
        {"cleared","ReRuns"},
    };
    static final String[][] WATCHED_OPTS=new String[][] {
        {"any","Any"},
        {"set","Watched"},
        {"cleared","Not Watched"},
    };
    static final String[][] PARTIAL_OPTS=new String[][] {
        {"none","Only Complete Recordings"},
        {"only","Only Partials"},
        {"both","Partials and Complete Recordings"},
    };
    static final String[][] SET_CLEAR_OPTS=new String[][] {
        {"any","Any"},
        {"set","Set"},
        {"cleared","Cleared"},
    };
    static final String[][] YES_NO_OPTS=new String[][] {
        {"yes","Yes"},
        {"no","No"},
    };

    static final String[][] SORT_OPTS=new String[][]{
        {"airdate_asc","Airing Date (earliest first)"},
        {"airdate_desc","Airing Date (latest first)"},
        {"origairdate_asc", "Original Airing Date (earliest first)"},
        {"origairdate_desc", "Original Airing Date (latest first)"},
        {"title_asc","Show Title/Album Name (a-z)"},
        {"title_desc","Show Title/Album Name Reversed (z-a)"},
        {"episode_asc","Epsiode/Track Name (a-z)"},
        {"episode_desc","Epsiode/Track Name Reversed (z-a)"},
        {"people_asc","People/Artist Name(a-z)"},
        {"people_desc","People/Artist Name Reversed (z-a)"},
        {"none","No sorting"}
    };
    static String[][] GROUPING_OPTS=new String[][]{
        {"##AsSageTV##","Same as SageTV"},
        {"None","No Grouping"},
        {"GetAiringTitle","Title/Album Name"},
