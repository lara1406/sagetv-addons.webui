...
public class VlcTranscodeMgr {
    ...
    // transcode properties are in:
    // transcodeMode/<format>/<name>=<params>
    
    // Map of format->list of modes
    // where a mode is a String[2] {name, params}
    // Map<String,List<String[2]>>
     ...
     */
    public TranscodeInputStream startTranscodeProcess(String localPath, String videoCodec, int videoBitrate,
                                                      String audioCodec, int audioBitrate, String scale, boolean deint, String mux) throws Throwable {
        readProperties();
        ...
        if (socket == null) {
            throw new IOException("Unable to allocate port for streaming " +
                    "between specified ports " + startPort + "-" + stopPort);
        }
        int port = socket.getLocalPort();

        //build command
        StringBuffer cmdBuff = new StringBuffer();
        // won't find unescaped quote at first character so add a space now then trim it later
        cmdBuff.append(" \"");
        ...
        if (debugLog)
	        Acme.Serve.Serve.extLog("OS: " + System.getProperty("os.name"));
        // Linux command line with quotes does not work when run from Java.  Remove the quotes
        // and escape the spaces that were inside the quotes.
        if (System.getProperty("os.name").toLowerCase().startsWith("linux")) {
			if (debugLog)
                Acme.Serve.Serve.extLog("Converting Command for Linux: " + cmdBuff);
            //String EQ = "\\\\" + "\\\""; // escaped quotes
            String UQ = "(?<=[^\\\\])\\\""; // unescaped quotes
            String US = "(?<=[^\\\\])\\s"; // unescaped space
            ...
            while (m.find())
            {
                ...
                // double escape everything for Java's interpreter
                s = s.replace("\\", "\\\\");
        ...
        synchronized (streams) {
            socket.close();
            if ( debugLog)
                Acme.Serve.Serve.extLog("Executing command: " + cmdBuff.substring(1));
            
            final Process proc = Runtime.getRuntime().exec(cmdBuff.toString().substring(1), null, vlcWkDir);
            // fire off process output gobblers
            if ( proc != null ) {
                // stdout
                Thread stdout_reader=new Thread() {
                    public void run() {
                        ...
                        String line;
                        try {
                            while ( null != ( line = proc_out.readLine() ) )
                                if ( debugLog)
                                    Acme.Serve.Serve.extLog("VLC(out): "+line);
                        } catch (IOException e) {}
                        try {
                            proc_out.close();
                        } catch (IOException e) {}
                ...
                Thread stderr_reader=new Thread() {
                    public void run() {
                        ...
                        String line;
                        try {
                            while ( null != ( line = proc_err.readLine() ) )
                                if ( debugLog)
                                    Acme.Serve.Serve.extLog("VLC(err): "+line);
                        } catch (IOException e) {}
                        try {
                            proc_err.close();
                        } catch (IOException e) {}
            ...
            try {
                //connect to port and get inputstream
                int retryCount = 0;
                ...
                while (retryCount < maxRetries) {
                    try {
                        ...
                        if ( debugLog)
                            Acme.Serve.Serve.extLog("Connected to "+conn.toString());
                        
                        is = new TranscodeInputStream(conn.getInputStream(), proc, port, fileExt, serverBuffer);
                        streams.add(is);
                        if ( debugLog)
                            Acme.Serve.Serve.extLog("Returning InputStream: " + is);
                        return is;
                    } catch (IOException ex) {
                        //unable to open stream, increment retryCount and wait
    ...
    private void readProperties() {
        ...
        if ( propsFile.canRead()){
            ...
            props.put(PROP_VLC_OPTS, "--no-sub-autodetect-file --quiet %LOCAL_PATH% :sout=#transcode{vcodec=%VIDEO_CODEC%," +
                    "vb=%VIDEO_BITRATE%,scale=%SCALE%,acodec=%AUDIO_CODEC%,ab=%AUDIO_BITRATE%,channels=2,threads=2%DEINTERLACE%}" +
                    ":duplicate{dst=std{access=http,mux=ts,dst=%LOCALHOST%:%PORT%}}");
        ...
        try {
            localhostAddr=java.net.InetAddress.getByName("127.0.0.1"); // should always succeed
        ...
        if ( debugLogStr == null 
             || ( ! debugLogStr.equalsIgnoreCase("true")
                  && ! debugLogStr.equals("1"))) 
            debugLog=false;
        else
            debugLog=true;
        ...
        if ( orbtest==null || orbtest.trim().equalsIgnoreCase("auto"))
            orbInstalled=new File("C:\\Program Files\\Orb Networks\\Orb\\bin\\OrbTray.exe").exists();
        else {
    ...
    public static void main(String[] args) {
        ...
        String mux=args[7];
        boolean deint=false;
        if ( args[6].equalsIgnoreCase("deint"))
            deint=true;

        VlcTranscodeMgr mgr = VlcTranscodeMgr.getInstance();
        InputStream is = null;
        try {
            is = mgr.startTranscodeProcess(localPath, videoCodec, Integer.parseInt(videoBitrate),
                            audioCodec, Integer.parseInt(audioBitrate), scale,deint,mux);
            ...
            try {
                ...
                //not a big deal.  Was probably closed already.
            }
        }
    }
    ...
    public boolean isVlcInstalled(){
        ...
        try {
            readProperties();
            if ( vlcCmd!= null && vlcCmd.trim().length()>0)
                return new File(vlcCmd).exists() ;
        } catch (Exception e){}
        return false;
    }
    /**
