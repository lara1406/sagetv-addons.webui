...
public class FavoriteCommandServlet extends SageServlet {
	...
	protected void doServletGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        String category = req.getParameter("category"); // only editable on new favorite in v4 stv
        String subCategory = req.getParameter("subcategory"); // not in v4 stv
        String keyword = req.getParameter("keyword");
        String person = req.getParameter("person"); // only editable on new favorite in v4 stv
        String roleForPerson = req.getParameter("roleforperson"); // not in v4 stv
        String title = req.getParameter("title"); // only editable on new favorite in v4 stv
        String run = req.getParameter("run");
        String[] channelIDs = req.getParameterValues("channelID");
        String network = req.getParameter("network"); // not in v4 stv
        ...
        String year = req.getParameter("year"); // not in v4 stv
		...
		try {
            if ((command.equals("Add")) || (command.equals("Update"))) {
                Favorite favorite = Favorite.save(
                        Integer.parseInt(id),
                        category,
                        subCategory,
                        keyword,
                        ...
                        quality,
                        isAutoConvert,
                        isDeleteAfterFavoriteAutomaticConversion,
                        favoriteAutomaticConversionFormat,
                        isFavoriteAutomaticConversionDestinationOriginalDir,
                        favoriteAutomaticConversionDestination,
                        parentalRating,
                        rated,
                        year,
                        day,
