...
public class Favorite {
    ...
    static final String[] PARENTAL_RATINGS = new String[] {
    ...
    static final String[] TIMES_AM_PM = new String[] {
        "12AM", "1AM", "2AM", "3AM", "4AM", "5AM", "6AM", "7AM", "8AM", "9AM", "10AM", "11AM",
        "12PM", "1PM", "2PM", "3PM", "4PM", "5PM", "6PM", "7PM", "8PM", "9PM", "10PM", "11PM"
    };

    static final String[] TIMES_24 = new String[] {
    ...
    public static void createFavoritePriority(Favorite higherPriorityFavorite, Favorite lowerPriorityFavorite, boolean moveHigherPriorityFavorite) throws Exception {
        ...
        favoritesList = SageApi.Api("Sort", new Object[] {favoritesList, Boolean.FALSE, "FavoritePriority"}); // sort ascending
    ...
    public int increasePriority() throws Exception {
        ...
        // find the favorite above this one
        for (int i = 0; i < SageApi.Size(favoritesList); i++) {
            int currentFavoriteID = SageApi.IntApi("GetFavoriteID", new Object[] {SageApi.GetElement(favoritesList, i)});
            ...
            {
                if (i == 0) {
                    // this favorite is already at the highest priority
                    return 0;
                }
    ...
    public int decreasePriority() throws Exception {
        ...
        // find the favorite below this one
        for (int i = 0; i < SageApi.Size(favoritesList); i++) {
            int currentFavoriteID = SageApi.IntApi("GetFavoriteID", new Object[] {SageApi.GetElement(favoritesList, i)});
            ...
            if (lowerPriorityFavoriteID == currentFavoriteID)
            {
                if (i == SageApi.Size(favoritesList) - 1) {
                    // this favorite is already at the lowest priority
                    return 0;
                }
    ...
    public boolean isAutomaticConversionSupported() throws Exception {
        ...
        try {
            ...
            // favorite auto conversion new in version 7
    ...
    /** Save the favorite */
    public static Favorite save(int id,
            String category,
            String subCategory,
            String keyword,
            String person,
            String roleForPerson,
            String title,
            Boolean firstRunsOnly,
            Boolean reRunsOnly,
            String channels,
            String network,
            String autoDelete,
            String keepAtMost,
            String startPad,
            String startPadOffsetType,
            String endPad,
            String endPadOffsetType,
            String quality,
            boolean isAutoConvert,
            boolean isDeleteAfterFavoriteAutomaticConversion,
            String favoriteAutomaticConversionFormat,
            boolean isFavoriteAutomaticConversionDestinationOriginalDir,
            String favoriteAutomaticConversionDestination,
            String parentalRating,
            String rated,
            String year,
            String day,
            String time,
            String favoritePriorityRelation,
            String relativeFavoriteId
            )
        throws Exception {
        ...
        if (sageFavorite == null) {
            sageFavorite = SageApi.Api("AddFavorite", new Object[] {title,
                    firstRunsOnly, reRunsOnly, category, subCategory, person, roleForPerson, rated,
                    year, parentalRating, network, null, Favorite.getTimeslot(day, time), keyword});
        }

        // AddFavorite does not work with channels (possibly a bug introduced in a recent Sage release around 6.3)
        // call UpdateFavorite to set the channel
        SageApi.Api("UpdateFavorite", new Object[] {sageFavorite, title,
                firstRunsOnly, reRunsOnly, category, subCategory, person, roleForPerson, rated,
                year, parentalRating, network, channels, Favorite.getTimeslot(day, time), keyword}); 
    ...
    public void printFavoriteTableCell(PrintWriter out,
                                       Favorite previousFavorite,
                                       Favorite currentFavorite,
                                       Favorite nextFavorite,
                                       boolean hascheckbox,
                                       boolean showfulldescription,
                                       boolean showfirstrunsandreruns,
                                       boolean showautodelete,
                                       boolean showkeepatmost,
                                       boolean showpadding,
                                       boolean showquality,
                                       boolean showratings,
                                       boolean showtimeslot,
       ...
       //note -- no spaces between <img and titletext to prevent word-wrapping
        ...
        if (previousFavorite != null) {
            String commandStr="FavoriteCommand?command=CreatePriority" +
                            "&amp;LowerId=" + previousFavorite.getID() +
                            "&amp;HigherId=" + currentFavorite.getID() + 
                            "&amp;returnto=Favorites";
            out.println("                <a onclick=\"return AiringCommand('"+commandStr+"','FavID"+id+"')\" href=\""+commandStr+"\">");
            out.print("                    <img alt=\"\" ");
            out.print("style=\"float:left;\" "); // necessary for up arrow to be positioned above the down arrow and vertically align them on IE and Firefox
        ...
        if (nextFavorite != null) {
            String commandStr="FavoriteCommand?command=CreatePriority" +
                    "&amp;LowerId=" + currentFavorite.getID() +
                    "&amp;HigherId=" + nextFavorite.getID() + 
                    "&amp;returnto=Favorites";
            out.println("                <a onclick=\"return AiringCommand('"+commandStr+"','FavID"+id+"')\" href=\""+commandStr+"\">");
            out.print("                    <img alt=\"\" ");
            out.print("style=\"float:left;\" "); // necessary for up arrow to be positioned above the down arrow and vertically align them on IE and Firefox
