    ...
    protected void doServletGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
            ...
            if ( req.getQueryString() != null)
                xmlurl.append(req.getQueryString().replaceAll("&","&amp;")).append("&amp;");
            xmlurl.append("xml=");
		    ...
		    if ( UiContexts.length>0 || (ConnectedClients.length > 0 && SAGE_MAJOR_VERSION >= 7.0)){ 
		        ...
		        if ( Arrays.asList(UiContexts).contains("SAGETV_PROCESS_LOCAL_UI")){
		            out.println("<h3>On Local SageTV instance:" +
                                "<a href=\""+xmlurl+"currplaying&amp;context=SAGETV_PROCESS_LOCAL_UI\" title=\"Return currently playing in XML\"><img src=\"xml_button.png\" alt=\"[XML]\"/></a>\r\n" +
                                "<a href=\"webremote.html?context=SAGETV_PROCESS_LOCAL_UI&amp;contextname=Local%20SageTV\" title=\"Webremote for local SageTV\" target=\"webremote\">[WebRemote]</a>"+
                                "</h3>");
		            Object sageAiring=SageApi.ApiUI("SAGETV_PROCESS_LOCAL_UI","GetCurrentMediaFile");
		            if ( sageAiring!=null) {
		                out.println("<div class=\"airings\">");
		                new Airing(sageAiring).printAiringTableCell(
		                        req,
		                        out,
		                        false,
		                        usechannellogos,
                                ...
                                null,
                                null);
                        if ( ! ((Boolean)SageApi.ApiUI("SAGETV_PROCESS_LOCAL_UI", "IsPlaying")).booleanValue())
                            out.print("(Paused) ");
		        ...
		        for ( int ui=0;ui<UiContexts.length;ui++){
		            ...
		            if ( ! context.equals("SAGETV_PROCESS_LOCAL_UI")) {
		                ...
		                out.println("<a href=\"webremote.html?context="+URLEncoder.encode(context,charset)
                                    +"&contextname="+URLEncoder.encode(UiContextProperties.getProperty(context, "name"),charset)
                                    +"\" target=\"webremote\" title=\"Webremote for Sage Extender\" >"
                                    +"[WebRemote]</a>\r\n");
		                if ( ExtenderCommandServlet.isExtender(context) ) {
			                out.println(" <a onclick=\"" +
			                			"confirmAction('"+"Are you sure you want to Power Off this Extender?"+"'," + 
			                			"'ExtenderCommand?context="+URLEncoder.encode(context,charset)+"&command=poweroff&returnto=Home&amp;confirm=yes');" +
			                			" return false;\"" +
			                			"href=\"ExtenderCommand?context="+URLEncoder.encode(context,charset)
			                			+"&command=poweroff&returnto=Home\" title=\"Power off this Extender\" >"
			                			+"[Power Off]</a>\r\n");
			                out.println(" <a onclick=\"" +
			                			"confirmAction('"+"Are you sure you want to Reboot this Extender?"+"'," + 
			                			"'ExtenderCommand?context="+URLEncoder.encode(context,charset)+"&command=reboot&returnto=Home&amp;confirm=yes');" +
			                			" return false;\"" +
			                			"href=\"ExtenderCommand?context="+URLEncoder.encode(context,charset)
	                                +"&command=reboot&returnto=Home\" title=\"Reboot this Extender\" >"
	                                +"[Reboot]</a>\r\n");
		                } else {
		                	out.println(" <a onclick=\"" +
		                			"confirmAction('"+"Are you sure you want to Exit this Placeshifter?"+"'," + 
		                			"'ExtenderCommand?context="+URLEncoder.encode(context,charset)+"&command=exit&returnto=Home&amp;confirm=yes');" +
		                			" return false;\"" +
		                			"href=\"ExtenderCommand?context="+URLEncoder.encode(context,charset)
		                			+"&command=exit&returnto=Home\" title=\"Tell this Placeshifter to Exit\" >"
		                			+"[Exit]</a>\r\n");
		                }
                        out.println("</h3>");
		                Object sageAiring=SageApi.ApiUI(context,"GetCurrentMediaFile");
		                if ( sageAiring!=null) {
		                    ...
		                    new Airing(sageAiring).printAiringTableCell(
                                    ...
                                    null,
                                    null);
                            if ( ! ((Boolean)SageApi.ApiUI(context, "IsPlaying")).booleanValue())
                                out.print("(Paused) ");
            ...
            // print next upcoming recordings.  if multiple recordings start at the same time, show them all
            ...
            if ((filelist != null) && (SageApi.Size(filelist) > 0)) {
                ...
                // get start time of next airing, ignoring padding
                long nextRecordingDateMillis = new Airing(SageApi.GetElement(filelist,0)).getStartMillis();
                Calendar nextRecordingDateCal = Calendar.getInstance(); // for diffs
                nextRecordingDateCal.setTimeInMillis(nextRecordingDateMillis);
                Date nextRecordingDate = new Date(nextRecordingDateMillis); // for formatting
                ...
                if ((today.get(Calendar.YEAR) == nextRecordingDateCal.get(Calendar.YEAR)) &&
                    (today.get(Calendar.MONTH) == nextRecordingDateCal.get(Calendar.MONTH)) &&
                    (today.get(Calendar.DAY_OF_MONTH) == nextRecordingDateCal.get(Calendar.DAY_OF_MONTH))) {
                    // today
                    out.println("<h3>Today at " +  timeFormat.format(nextRecordingDate) + "</h3>");
                } else if ((tomorrow.get(Calendar.YEAR) == nextRecordingDateCal.get(Calendar.YEAR)) &&
                           (tomorrow.get(Calendar.MONTH) == nextRecordingDateCal.get(Calendar.MONTH)) &&
                           (tomorrow.get(Calendar.DAY_OF_MONTH) == nextRecordingDateCal.get(Calendar.DAY_OF_MONTH))) {
                        // tomorrow
                ...
                for (int i = 0; i < SageApi.Size(filelist); i++) {
                    ...
                    for ( int j=0; j<SageApi.Size(encoders); j++){
                        ...
                        Object encoderAirings=SageApi.Api("GetScheduledRecordingsForDeviceForTime",
                    ...
                    out.println("<div class=\"airings\">");
                    airing.printAiringTableCell(req,out,false,
                            usechannellogos,
                            showMarkers,
                            showRatings,
                            showEpisodeID,
            ...
            if ( numconflicts>0){
                ...
                if ( numresolved>0)
                    out.println("<img src=\"resolvedconflicticon.gif\" alt=\"Resolved Conflict\" title=\"Resolved Conflict\"/>" +numresolved+" Resolved Conflicts</a></h2>");
                out.println("</a></h2><hr/>");
            }
		    ...
		    if ( System.currentTimeMillis() - LastUpdate > 60*1000 )
		        out.println("<p><a href=\"GlobalCommand?command=ForceEpgUpdate\" title=\"Click this to force an EPG update to occur immediately on all lineups\">[Force EPG update]</a></p>");
		    ...
		    out.println("<ul>");
		    {
			    ...
			    out.println("<li>Music: " 
		    			+ SageApi.Size(SageApi.Api("GetAlbums"))+" albums / "
		    			+ SageApi.Size(mediafiles)+" tracks"
		    			+"</li>");
			    ...
			    out.println("<li>Video: " + SageApi.Size(mediafiles)+" files"
		    			+"</li>");
			    
			    mediafiles=SageApi.Api("FilterByBoolMethod",new Object[]{libraryfiles,"IsDVD", Boolean.TRUE});
		    	out.println("<li>DVDs: " + SageApi.Size(mediafiles)+" files"
		    			+"</li>");

		    	
			    mediafiles=SageApi.Api("FilterByBoolMethod",new Object[]{libraryfiles,"IsPictureFile", Boolean.TRUE});
			    out.println("<li>Pictures: "+ SageApi.Size(mediafiles)+" files"
		    			+"</li>");
		    	
		    	out.println("<li>Total Media Library Duration: "+SageApi.Api("PrintDuration",SageApi.Api("GetTotalLibraryDuration"))
		    				+"</li>");
		    }
